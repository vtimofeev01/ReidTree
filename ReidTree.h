...
//            printf("nodes:%i\n queue:%lu", node_calculated, p_queue.size());
    ...
    public:
        ...
        T Node2NodeCompare(const pTBNode&  n1, const pTBNode&  n2){
            auto nid1 = std::min(n1->id, n2->id);
            auto nid2 = std::max(n1->id, n2->id);
            auto ix = std::make_pair(nid1, nid2);
        ...
        int add_idents_to_tree(std::vector<T> &data_, bool use_max_pass = true) {
            ...
            while (pointer != nullptr) { // !cur_node_list->empty()
                ...
                better_to_left = new_to_left_cs > new_to_right_cs;
                if (goes_to_child && better_to_left) {
                    ...
                    auto tmp_ptr = pointer->left;
                    pointer = insert_node;
                    insert_node = tmp_ptr;
                    continue;
                } else {
                    add_btree_vector_log("   <%i> is set to right. <%i> pushed up. cross cs: %f -> %f ", insert_node->id, pointer->right->id, pointer->cross_sim, new_to_left_cs);
                    pointer->cross_sim = new_to_left_cs;
                    auto tmp_ptr = pointer->right;
                    pointer = insert_node;
                    insert_node = tmp_ptr;
...
#define CD_block \
        ...
        if (sim > max_similarity) max_similarity = sim; \
        if (sim > similarity_for_same) return 2; \
        if (sim > parent_limit && node1->isNode() && node2->isNode()) p_queue.push(std::make_pair(std::make_pair(cur1, cur2), sim));

        T to_tree(std::shared_ptr<BTree> b, T similarity_for_same=.95) {
            // fill with roots
            ...
            while (!p_queue.empty()) {
                ...
                tree_comp_log("\ngot [%i %i] similarity: %f   calculated:%i queue:%lu crosses:(1) %f (2) %f\n",
                              node1->id, node2->id, v.second,
